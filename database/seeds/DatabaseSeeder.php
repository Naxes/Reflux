<?php

use App\User;
use App\Post;
use App\Tag;
use App\Comment;
use Illuminate\Database\Seeder;

class DatabaseSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        $this->call(UsersTableSeeder::class);
        $this->call(PostsTableSeeder::class);
        $this->call(TagsTableSeeder::class);
        $this->call(CommentsTableSeeder::class);
    }
}

class UsersTableSeeder extends Seeder 
{
    public function run() 
    {
        DB::table('users')->delete();

        User::create(
        [
            'name'      => 'Seán',
            'email'     => 'seanbickmore@icloud.com',
            'password'  => Hash::make('tester'),
            'bio'       => 'Welcome to Reflux, my fourth year Cyber Security project!',
            'url'       => 'linkedin.com/in/seánbickmore'
        ]);

        User::create(
        [
            'name'      => 'Tom',
            'email'     => 'tom@example.com',
            'password'  => Hash::make('tester')
        ]);

        User::create(
        [
            'name'      => 'Sarah',
            'email'     => 'sarah@example.com',
            'password'  => Hash::make('tester')
        ]);

        User::create(
        [
            'name'      => 'Donald',
            'email'     => 'donald@example.com',
            'password'  => Hash::make('tester')
        ]);

        User::create(
        [
            'name'      => 'Glen',
            'email'     => 'Tulki@example.com',
            'password'  => Hash::make('tester')
        ]);
    }
}

class PostsTableSeeder extends Seeder 
{
    public function run()
    {
        DB::table('posts')->delete();

        Post::create(
        [
            'title'     => 'HTML',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>Markup Post!</code></pre>',
            'score'     => 1250,  
            'user_id'   => 1
        ]);

        Post::create(
        [
            'title'     => 'CSS',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>CSS Post!</code></pre>',
            'score'     => 671,  
            'user_id'   => 2
        ]);
        
        Post::create(
        [
            'title'     => 'Sass (Sass)',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>Sass (Sass) Post!</code></pre>',
            'score'     => 671,  
            'user_id'   => 1
        ]);

        Post::create(
        [
            'title'     => 'Sass (Scss)',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>Sass (Scss) Post!</code></pre>',
            'score'     => 671,  
            'user_id'   => 3
        ]);

        Post::create(
        [
            'title'     => 'JavaScript',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>JavaScript Post!</code></pre>',
            'score'     => 1500,  
            'user_id'   => 1
        ]);

        Post::create(
        [
            'title'     => 'PHP',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>PHP Post!</code></pre>',
            'score'     => 2350,  
            'user_id'   => 4
        ]);

        Post::create(
        [
            'title'     => 'SQL',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>SQL Post!</code></pre>',
            'score'     => 2350,  
            'user_id'   => 1
        ]);

        Post::create(
        [
            'title'     => 'Java',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>Java Post!</code></pre>',
            'score'     => 436,  
            'user_id'   => 5
        ]);       

        Post::create(
        [
            'title'     => 'Ruby',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>Ruby Post!</code></pre>',
            'score'     => 781,  
            'user_id'   => 1
        ]);

        Post::create(
        [
            'title'     => 'Python',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>Python Post!</code></pre>',
            'score'     => 229,  
            'user_id'   => 2
        ]);

        Post::create(
        [
            'title'     => 'C#',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>C# Post!</code></pre>',
            'score'     => 404,  
            'user_id'   => 1
        ]);

        Post::create(
        [
            'title'     => 'Objective-C',
            'body'      => '<p>This is a post generated by <b>seeding</b> to populate the site.</p>
                            <pre class="language-markup"><code>Objective-C Post!</code></pre>',
            'score'     => 515,  
            'user_id'   => 3
        ]);        
    }
}

class TagsTableSeeder extends Seeder
{
    public function run()
    {
        DB::table('tags')->delete();

        Tag::create(
        [
            'post_id' => 1,
            'name'    => 'HTML/XML'
        ]);

        Tag::create(
        [
            'post_id' => 2,
            'name'    => 'CSS'
        ]);

        Tag::create(
        [
            'post_id' => 3,
            'name'    => 'Sass (Sass)'
        ]);

        Tag::create(
        [
            'post_id' => 4,
            'name'    => 'Sass (Scss)'
        ]);

        Tag::create(
        [
            'post_id' => 5,
            'name'    => 'JavaScript'
        ]);

        Tag::create(
        [
            'post_id' => 6,
            'name'    => 'PHP'
        ]);

        Tag::create(
        [
            'post_id' => 7,
            'name'    => 'SQL'
        ]);

        Tag::create(
        [
            'post_id' => 8,
            'name'    => 'Java'
        ]);

        Tag::create(
        [
            'post_id' => 9,
            'name'    => 'Ruby'
        ]);

        Tag::create(
        [
            'post_id' => 10,
            'name'    => 'Python'
        ]);

        Tag::create(
        [
            'post_id' => 11,
            'name'    => 'C#'
        ]);

        Tag::create(
        [
            'post_id' => 12,
            'name'    => 'Objective-C'
        ]);
    }
}

class CommentsTableSeeder extends Seeder
{
    public function run()
    {
        DB::table('comments')->delete();

        Comment::create(
        [
            'user_id' => 2,
            'post_id' => 1,            
            'comment' => "Nice one, pretty cool solution"
        ]);

        Comment::create(
        [
            'user_id'   => 3,
            'post_id'   => 1,
            'parent_id' => 1,           
            'comment'   => "I agree! Great post OP"
        ]);

        Comment::create(
        [
            'user_id'   => 1,
            'post_id'   => 1,
            'parent_id' => 1,           
            'comment'   => "Thanks guys!"
        ]);

        Comment::create(
        [
            'user_id' => 4,
            'post_id' => 1,            
            'comment' => "I think there might be an error"
        ]);

        Comment::create(
        [
            'user_id'   => 1,
            'post_id'   => 1,
            'parent_id' => 4,           
            'comment'   => "Where abouts do you see it?"
        ]);

        Comment::create(
        [
            'user_id'   => 5,
            'post_id'   => 1,
            'parent_id' => 4,           
            'comment'   => "I think he means on line 5"
        ]);

        Comment::create(
        [
            'user_id'   => 1,
            'post_id'   => 1,
            'parent_id' => 4,           
            'comment'   => "Okay, I'll edit the post soon, just preoccupied atm"
        ]);
    }
}