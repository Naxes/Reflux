    ...
    <div class="ui move reveal">
        ...
        <div class="hidden content">        
            <form action="/vote/{{ $post->id }}" method="post" postid="{{ $post->id }}">
                ...
                <button class="ui icon {{ $post->votes->where('user_id', auth()->id())->where('post_id', $post->id)->first() ? 'blue' : 'basic blue' }} button unvote vote_type_{{ $post->id }}" style="margin: 2px 0 0 4px;">
